...
class Messages:
	...
	username_instruction = "Enter the username as username@{}"
...
class InstallerData:
	...
	def configure_profile(self) -> None:
		"""
		Allow the user to configure university profile settings.
		"""
		if self.silent:
			return
		...
		while True:
			if self.graphics == 'tkinter':
				...
				root = tk.Tk()
				root.title(Messages.profile_config_title)

				tk.Label(root, text=Messages.inst_name_label).grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
				inst_entry = tk.Entry(root, width=40)
				inst_entry.insert(0, Config.instname)
				inst_entry.grid(row=0, column=1, padx=5, pady=2)

				tk.Label(root, text=Messages.user_realm_label).grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
				realm_entry = tk.Entry(root, width=40)
				realm_entry.insert(0, Config.user_realm)
				realm_entry.grid(row=1, column=1, padx=5, pady=2)

				tk.Label(root, text=Messages.anon_id_label).grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
				anon_entry = tk.Entry(root, width=40)
				anon_entry.insert(0, Config.anonymous_identity)
				anon_entry.grid(row=2, column=1, padx=5, pady=2)

				tk.Label(root, text=Messages.ssids_label).grid(row=3, column=0, sticky=tk.W, padx=5, pady=2)
				ssids_entry = tk.Entry(root, width=40)
				ssids_entry.insert(0, ",".join(Config.ssids))
				ssids_entry.grid(row=3, column=1, padx=5, pady=2)

				def save():
					Config.instname = inst_entry.get()
					Config.user_realm = realm_entry.get()
					Config.anonymous_identity = anon_entry.get()
					Config.ssids = [s.strip() for s in ssids_entry.get().split(",") if s.strip()]
					root.destroy()

				tk.Button(root, text=Messages.ok, command=save).grid(row=4, column=0, columnspan=2, pady=10)
				root.mainloop()
				break

			elif self.graphics == 'yad':
				args = ['--form',
						f"--field={Messages.inst_name_label}:ENTRY",
						f"--field={Messages.user_realm_label}:ENTRY",
						f"--field={Messages.anon_id_label}:ENTRY",
						f"--field={Messages.ssids_label}:ENTRY",
						'--button=Cancel:1', '--button=OK:0',
						"--separator", output_fields_separator,
						Config.instname, Config.user_realm, Config.anonymous_identity, ",".join(Config.ssids)]

				rc, output = self._yad_cmd(args, title=Messages.profile_config_title, image='preferences-system')
				if rc == 1:
					self.confirm_exit()
					continue
				fields = output.split(output_fields_separator)
				if len(fields) >= 4:
					Config.instname, Config.user_realm, Config.anonymous_identity, ssids_str = fields[:4]
					Config.ssids = [s.strip() for s in ssids_str.split(",") if s.strip()]
				break

			elif self.graphics == 'zenity':
				command = ['zenity', '--forms', '--width=500',
						   f"--title={Messages.profile_config_title}",
						   f"--add-entry={Messages.inst_name_label}",
						   f"--add-entry={Messages.user_realm_label}",
						   f"--add-entry={Messages.anon_id_label}",
						   f"--add-entry={Messages.ssids_label}",
						   "--separator", output_fields_separator]
				...
				fields = output.split(output_fields_separator)
				if len(fields) >= 4:
					Config.instname = fields[0]
					Config.user_realm = fields[1]
					Config.anonymous_identity = fields[2]
					Config.ssids = [s.strip() for s in fields[3].split(",") if s.strip()]
				break

			else: # TTY
				print(f"\n--- {Messages.profile_config_title} ---")
				Config.instname = self.__prompt_with_default(Messages.inst_name_label, Config.instname)
				Config.user_realm = self.__prompt_with_default(Messages.user_realm_label, Config.user_realm)
				Config.anonymous_identity = self.__prompt_with_default(Messages.anon_id_label, Config.anonymous_identity)
				ssids_str = self.__prompt_with_default(Messages.ssids_label, ",".join(Config.ssids))
				Config.ssids = [s.strip() for s in ssids_str.split(",") if s.strip()]
				break
...
class IwdConfiguration:
	...
	def write_config(self, ssid: str, force_system_path: bool = False) -> None:
		...
		if permissions are insufficient, safely use pkexec tee to write to secure paths.
		"""
...
Messages.username_prompt = "Username"
Messages.username_instruction = "Enter the username as username@{}"
Messages.profile_config_title = "Configure University Profile"
Messages.inst_name_label = "Institution Name"
Messages.user_realm_label = "User Realm"
Messages.anon_id_label = "Anonymous Identity"
Messages.ssids_label = "Network SSID"
...
Messages.cat_dir_exists = "Directory {} exists; some of its files may " \
	"be overwritten"
Config.instname = "University of Peloponnese"
Config.profilename = "UoP eduroam configuration"
Config.url = "http://di.uop.gr/"
Config.email = "helpdesk@uop.gr"
Config.title = "eduroam CAT"
Config.server_match = "ridp.uop.gr"
...
Config.user_realm = "uop.gr"
Config.ssids = ['eduroam']
Config.del_ssids = []
Config.servers = ['DNS:ridp.uop.gr']
Config.servers_cn = 'ridp.uop.gr'
Config.use_other_tls_id = False
Config.anonymous_identity = "anonymous@uop.gr"
Config.tou = ""
Config.CA = """-----BEGIN CERTIFICATE-----

C11T8Ngs5biSrGL0SytQ9a6B
-----END CERTIFICATE-----
"""
