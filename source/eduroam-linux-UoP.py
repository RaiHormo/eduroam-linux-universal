...
class InstallerData:
    ...
    def __get_username_password_atomic(self) -> None:
        ...
        while True:
            display_username = self.username if self.username else f"username@{Config.user_realm}"
            placeholder = f"username@{Config.user_realm}"
            password = "a"
            password1 = "b"
            if self.graphics == 'tkinter':
                ...
                username_entry = tk.Entry(root, textvariable=tk.StringVar(root, value=display_username))
                ...
                if self.graphics == 'zenity':
                    ...
                    command = ['yad', '--form',
                               ...
                               f"--field={Messages.username_prompt}", display_username,
                               f"--field={Messages.username_prompt}!!{placeholder}",
                               f"--field={Messages.enter_password}:H",
                               f"--field={Messages.repeat_password}:H",
                               "--separator", output_fields_separator]
                               "--separator", output_fields_separator, self.username, "", ""]
